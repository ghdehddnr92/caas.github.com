...
챗봇의 Speak Node 나 Slot Node 에 피자가격을 직접 입력하여 사용하면 기존 시스템에서 가격이 변경 될 경우 가격을 여러번 수정해야하는 요소가 생깁니다. <br/>
...
우상단의 ***“API 등록”*** 버튼을 눌러 API 생성화면으로 들어갑니다. <br/>

{% include image.html file="dapTalk/tutorial/basic07_API_01.jpg" max-width="900" %}
...
5) ***“API 설명”***을 입력합니다.
튜토리얼에서는 ***“피자주문 튜토리얼에서 피자메뉴에 따른 피자가격을 가지고 오는 API 입니다.”*** 를 입력하겠습니다. <br/>

{% include image.html file="dapTalk/tutorial/basic07_API_02.jpg" max-width="900" %}
...
***“http://dap.api.lgcns.com:8081/chatflow/pizzatutorial.do”***
...
q, s 라는 변수에 피자메뉴를 넘겨준다고 미리 정의 하였기 때문에 Name 부분에 q,s 를 각각 입력합니다. <br />
q 의 value에는 피자 종류 중 하나 인 "페퍼로니", s 의 value에는 피자의 사이즈인 "M"을 입력합니다. <br />
그리고 ***“실행”*** 버튼을 클릭합니다. <br/>

{% include image.html file="dapTalk/tutorial/basic07_API_03.jpg" max-width="900" %}
...
{% include image.html file="dapTalk/tutorial/basic07_API_04.jpg" max-width="900" %}
...
3) 기존에 연결된 선의 경우 마우스로 선을 클릭하고 선이 붉은색으로 하이라이트된 후 키보드의 delete 키를 누르면 삭제됩니다. <br/>

{% include image.html file="dapTalk/tutorial/basic07_API_05.jpg" max-width="900" caption="기존 연결선 삭제하기"%}

 <br/>

4) Chatflow 상단의 Api Node 를 클릭하여 추가해 주세요.

5) Carausel Node 와 Split Node 사이에 Api Node를 위치 시키고 앞뒤 노드와 연결해 주세요. <br/>
기왕이면 알아보기 쉽도록 ***“외부에서 피자가격 가져오기”*** 와 같은 노드명을 부여하면 좋겠죠? <span style="color:#2c3238;"><i class="fa fa-external-link-square" aria-hidden="true" style="margin: 0px 5px"></i>[Node명 수정하기](/basic_listen_speak.html#node-%EB%AA%85-%EC%88%98%EC%A0%95)</span>

{% include image.html file="dapTalk/tutorial/basic07_API_06.jpg" max-width="900" caption="API Node 생성하기" %}

#### API Node 상세정보 설정하기(요청 Parameter)
1) Api Node를 더블 클릭하여 상세화면으로 이동해주세요. 상세화면의 ***“API 설정”*** 탭이 열립니다. <br/>

{% include image.html file="dapTalk/tutorial/basic07_API_07.jpg" max-width="900" caption="API Node 생성하기2" %}

2) ***“사용 API”*** 부분에 ***“선택하세요”*** 를 누르면 현재 테넌트에서 사용 가능한 API 목록이 하단에 펼쳐집니다. <br/>
앞에서 만들었던 ***“피자주문하기_api_피자가격 가져오기”*** 가 보일것 입니다. 

3) ***“피자주문하기_api_피자가격 가져오기”*** 를 선택해 주세요. 선택 후 적용을 누르면 앞에서 설정한 내역을 불러 옵니다. <br/>

{% include image.html file="dapTalk/tutorial/basic07_API_08.jpg" max-width="900" caption="API Node 생성하기3" %}

4) 불러온 부분의 ***“요청 Parameter”*** 탭의 ***“Query Parameter”*** 부분에 요청시 보낼 Parameter 를 설정합니다. <br/>
외부시스템은 대화흐름에서 사용자가 선택한 #{피자메뉴} 변수를 받아 해당 메뉴에 해당하는 가격을 보내 줄 것입니다. <br/>
앞서서 q 라는 변수에 피자메뉴를 넘겨주기로 약속했기 때문에 사용자가 선택한 피자메뉴가 담겨있는 내부 변수 #{피자메뉴}를 q 에 입력 하도록 하겠습니다. <br/>
s 라는 변수에는 피자 사이즈인 M 값을 입력 하도록 하겠습니다. <br/>

{% include image.html file="dapTalk/tutorial/basic07_API_09.jpg" max-width="900" caption="API Node 생성하기4" %}


#### API Node 상세정보 설정하기(응답 Parameters)
1) 외부시스템에서 받아온 값을 대화흐름에서 사용하기 위해서는 외부시스템에서 받은 결과를 대화흐름 내부의 변수에 Mapping 시켜줘야 합니다. <br/>
이 작업 진행을 위해  ***“응답 Parameters”*** 탭으로 이동합니다. <br/>

2) ***“응답 Parameters”*** 섹션 우측의 + 버튼을 누르면 대화흐름 내부에서 정의한 변수들이 나타납니다. <br/>
피자가격을 받아오기 위해 대화흐름 내부에서 정의한 피자가격 변수를 선택합니다. 피자가격 변수가 보이지 않을 경우 <br/>
피자가격 변수를 만들어 주세요. 만드는 방법은 이전 챕터에서 설명한 내용을 보시기 바랍니다.
<span style="color:#2c3238;"><i class="fa fa-external-link-square" aria-hidden="true" style="margin: 0px 5px"></i>[Parameter 추가하기](/basic_entity_parameter.html#parameter-추가)</span>
...
{% include image.html file="dapTalk/tutorial/basic07_API_10.jpg" max-width="900" caption="API Node 생성하기5" %}
...
{% include image.html file="dapTalk/tutorial/basic07_API_11.jpg" max-width="900" caption="API Node test1" %} 

2) 우측의 테스트 패널에서 테스트를 해보세요. “피자 주문해줘”라고 입력 후 피자메뉴 선택 하면 <br/>
Speak 노드에 #{피자가격} 을 넣어 만든 문구를 확인하실 수 있습니다.

{% include image.html file="dapTalk/tutorial/basic07_API_12.jpg" max-width="900" caption="API Node test2" %} 
